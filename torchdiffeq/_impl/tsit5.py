...
def _optimal_step_size(last_step, mean_error_ratio, safety=0.9, ifactor=10.0, dfactor=0.2, order=5):
    ...
    if mean_error_ratio < 1:
        dfactor = _convert_to_tensor(1, dtype=torch.get_default_dtype(), device=mean_error_ratio.device)
...
class Tsit5Solver(AdaptiveStepsizeODESolver):
    ...
    def __init__(
        ...
        **unused_kwargs
        ...
        self.safety = _convert_to_tensor(safety, dtype=torch.get_default_dtype(), device=y0[0].device)
        self.ifactor = _convert_to_tensor(ifactor, dtype=torch.get_default_dtype(), device=y0[0].device)
        self.dfactor = _convert_to_tensor(dfactor, dtype=torch.get_default_dtype(), device=y0[0].device)
